...
class Kconfig:
    ...
    def __init__(
        ...
        suppress_traceback (default: False):
          Helper for tools. When True, any OSError or KconfigError
          ...
          Other exceptions besides OSError and KconfigError are still
        ...
        except (OSError, KconfigError) as e:
    ...
    def _init(
        ...
        self._unset_match = _re_match(rf"# {escaped_prefix}([^ ]+) is not set")
    ...
    def load_config(self, filename=None, replace=True, verbose=None):
        ...
        (OSError). KconfigError is never raised here.
                ...
                if match:
                    ...
                    if sym.orig_type in _BOOL_TRISTATE:
                        ...
                        if not (
                            ...
                            self._warn(
                                f"'{val}' is not a valid value for the {TYPE_TO_STR[sym.orig_type]} "
                    ...
                    if not match:
                        ...
                        if line and not line.lstrip().startswith("#"):
                            self._warn(f"ignoring malformed line '{line}'", loc)
    ...
    def load_allconfig(self, filename):
        ...
        def std_msg(e):
            # "Upcasts" a _KconfigIOError to an OSError, removing the custom
            # __str__() message. The standard message is better here.
            return OSError(e.errno, e.strerror, e.filename)
        ...
        if allconfig in ("", "1"):
            try:
                ...
                try:
                    ...
                    sys.exit(
                        f"error: KCONFIG_ALLCONFIG is set, but neither {filename} "
    ...
    def _open_config(self, filename):
        ...
        try:
            ...
            raise _KconfigIOError(
                ...
                f"Check that the $srctree environment variable "
                f"({srctree_status}) is set correctly.",
            ) from e
    ...
    def _fn_val(self, args):
        ...
        if fn in self._functions:
            ...
            if len(args) - 1 < min_arg or (
                ...
                raise KconfigError(
                    f"{self.filename}:{self.linenr}: bad number of arguments in call "
    ...
    def _parse_block(self, end_token, parent, prev):
        ...
        if end_token:
            if end_token is _T_ENDCHOICE:
                ...
                end_str = "endmenu"
            raise KconfigError(
                f"error: expected '{end_str}' at end of '{self.filename}'"
            )
    ...
    def _parse_props(self, node):
        ...
        while self._next_line():
            ...
            if t0 in _TYPE_TOKENS:
                ...
                node.dep = self._make_and(node.dep, self._parse_depends())
                ...
                if node.item is not self.modules:
                    self._warn(
                        "the 'modules' property is not supported. Let "
                ...
                for rel, rel_name in (
                    ...
                    for target_sym, _, _ in rel:
                        if target_sym.orig_type not in _BOOL_TRISTATE_UNKNOWN:
                            self._warn(
                                f"{sym.name_and_loc} {rel_name} the {TYPE_TO_STR[target_sym.orig_type]} symbol {target_sym.name_and_loc}, which is not "
                                "bool or tristate"
                            )
                ...
                if sym.orig_type not in _INT_HEX:
                    ...
                    for low, high, _, _ in sym.ranges:
                        if not num_ok(low, sym.orig_type) or not num_ok(
                            ...
                            self._warn(
                                f"the {TYPE_TO_STR[sym.orig_type]} symbol {sym.name_and_loc} has a non-{TYPE_TO_STR[sym.orig_type]} "
    ...
    def _check_choice_sanity(self):
        ...
        def warn_select_imply(sym, expr, expr_type):
            msg = (
                f"the choice symbol {sym.name_and_loc} is {expr_type} by the following symbols, but "
        ...
        for choice in self.unique_choices:
            ...
            for default, _, _ in choice.defaults:
                ...
                if default.choice is not choice:
                    self._warn(
                        f"the default selection {default.name_and_loc} of {choice.name_and_loc} is not "
    ...
    def _check_undef_syms(self):
        ...
        for sym in self.syms.values():
            ...
            if not sym.nodes and not is_num(sym.name) and sym.name != "MODULES":
                ...
                parts = [f"undefined symbol {sym.name}:"]
                for node in sym_to_nodes.get(sym, ()):
                    parts.append(
                        f"\n\n- Referenced at {node.loc[0]}:{node.loc[1]}:\n\n{node}"
                    )
...
class Symbol:
        ...
        if self.orig_type in _INT_HEX:
            ...
            if vis and self.user_value:
                ...
                if has_active_range and not low <= user_val <= high:
                    ...
                    self.kconfig._warn(
                        f"user value {num2str(user_val)} on the {TYPE_TO_STR[self.orig_type]} symbol {self.name_and_loc} clamped to {num2str(clamp_to)} due "
                        "to being outside the active range "
                        f"([{num2str(low)}, {num2str(high)}])"
                    )
                else:
    ...
    def __init__(self):
        ...
        self.user_loc = self.user_value = self.choice = self.env_var = self._origin = (
...
class Choice:
    ...
    def __init__(self):
        ...
        self.name = self.user_value = self.user_selection = self.user_loc = (
            self._origin
    ...
    def _invalidate(self):
        self.user_loc = self._origin = self._cached_vis = self._cached_assignable = None
        self._cached_selection = _NO_CACHED_SELECTION
...
class MenuNode:
    ...
    def orig_defaults(self):
        ...
        See the class documentation.
        """
        return [(default, self._strip_dep(cond)) for default, cond, _ in self.defaults]
    ...
    def orig_selects(self):
        ...
        See the class documentation.
        """
        return [(select, self._strip_dep(cond)) for select, cond, _ in self.selects]
    ...
    def orig_implies(self):
        ...
        See the class documentation.
        """
        return [(imply, self._strip_dep(cond)) for imply, cond, _ in self.implies]
    ...
    def orig_ranges(self):
        ...
        """
        return [
            (low, high, self._strip_dep(cond)) for low, high, cond, _ in self.ranges
        ]
...
# Workaround:
#
# If 'errno' and 'strerror' are set on OSError, then __str__() always returns
...
class _KconfigIOError(OSError):
    def __init__(self, ioerror, msg):
        self.msg = msg
        super().__init__(ioerror.errno, ioerror.strerror, ioerror.filename)
...
def _locs(sc):
    ...
    if sc.nodes:
        return "(defined at {})".format(
            ", ".join("{}:{}".format(*node.loc) for node in sc.nodes)
        )
    ...
    for item in loop:
        ...
        if item.__class__ is Symbol:
            ...
            if item.weak_rev_dep is not item.kconfig.n:
                msg += (
                    f"(imply-related dependencies: {expr_str(item.weak_rev_dep)})\n\n"
                )
