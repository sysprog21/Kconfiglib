...
def _update_stats():
    ...
    _stats_label["text"] = f"Symbols: {total_syms} | Changed: {changed_syms}"
...
def _set_val_dialog(node, parent):
    ...
    range_info = uicommon.range_info(sym)
    if range_info:
...
def _check_valid(dialog, entry, sym, s):
    ...
    try:
        ...
        messagebox.showerror(
            "Bad value",
            f"'{s}' is a malformed {TYPE_TO_STR[sym.orig_type]} value",
            parent=dialog,
        )
        entry.focus_set()
        return False
...
def _leave_menu():
    ...
    if _cur_menu is not _kconf.top_node:
        old_menu = _cur_menu
        _enter_menu(uicommon.parent_menu(old_menu))
...
def _on_quit(_=None):
    ...
    while True:
        ...
        if not ync:
            ...
            if config_exists:
                msg = f"Configuration ({_conf_filename}) was not saved"
            _quit(msg)
            return
...
def _try_save(save_fn, filename, description):
    ...
    try:
        ...
        messagebox.showerror(
            ...
            f"Error saving {description} to '{e.filename}': {e.strerror} (errno: {errno.errorcode.get(e.errno, e.errno)})",
        )
        return False
...
def _try_load(filename):
    ...
    try:
        ...
        messagebox.showerror(
            ...
            f"Error loading '{filename}': {e.strerror} (errno: {errno.errorcode.get(e.errno, e.errno)})",
        )
        return False
...
def _jump_to_dialog(_=None):
    ...
    def interactive_search(_=None):
        ...
        _search_after_id = dialog.after(150, search)
    ...
    def jump_to_selected(event=None):
        ...
        if node not in _shown_menu_nodes(uicommon.parent_menu(node)):
            _show_all_var.set(True)
            if not _single_menu:
    ...
    dialog.geometry(f"+{_root.winfo_rootx() + 50}+{_root.winfo_rooty() + 50}")
...
def _jump_to(node):
    ...
    if _single_menu:
        _enter_menu(uicommon.parent_menu(node))
    else:
        _load_parents(node)
        ...
        if dep is not _kconf.n:
            for val, prefix, suffix in (
                ...
                s += sis(
                    dep,
                    val,
                    f"Symbols currently {prefix}{verb} this symbol{suffix}:\n",
                )
...
def _kconfig_def_info(item):
    ...
    for node in nodes:
        s += (
            ...
            f"{node.custom_str(uicommon.name_and_val_str)}"
        )
