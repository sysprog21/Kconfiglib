...
def run_selftests():
    ...
    verify_equal(_ordered_unique([1, 2, 3, 2, 1, 2, 3, 4, 3, 2, 1, 0]), [1, 2, 3, 4, 0])
    ...
    def verify_eval(expr, val):
        ...
        verify(res == val, "'{}' evaluated to {}, expected {}".format(expr, res, val))
    ...
    def verify_custom_str(item, s):
        verify_equal(item.custom_str(lambda sc: "[{}]".format(sc.name)), s[1:-1])
...
""",
    ...
    def verify_deps(elms, dep_index, expected):
        verify_equal(" ".join(expr_str(elm[dep_index]) for elm in elms), expected)
...
""",
    ...
    print(
        "Testing locations, origins, source/rsource/gsource/grsource, "
...
"""[:-1],
    ...
    verify_equal(
        ...
        ["one-def choice", "two-def choice 1", "two-def choice 2", "menu", "comment"],
    )
    ...
    verify_sym_path("TWO_DOWN", 0, ("Kinclude_path", 4), ("Kinclude_path_sourced_1", 4))
    verify_sym_path("TWO_DOWN", 1, ("Kinclude_path", 4), ("Kinclude_path_sourced_1", 9))
    verify_sym_path("TWO_DOWN", 2, ("Kinclude_path", 9), ("Kinclude_path_sourced_1", 4))
    verify_sym_path("TWO_DOWN", 3, ("Kinclude_path", 9), ("Kinclude_path_sourced_1", 9))
    ...
    verify_equal(expr_str(c.named_choices["DEP_CHOICE"].direct_dep), "A || B || C")
    ...
    verify_equal(expr_str(c.syms["COND_DEP_MIXED"].direct_dep), "A && (!C || B) && D")
    ...
    verify_equal(expr_str(c.syms["MULTI_COND"].direct_dep), "(!B || A) && (!D || C)")
    ...
    def verify_expr_items(expr, *sym_names):
        verify_equal(tuple(sorted(item.name for item in expr_items(expr))), sym_names)
    ...
    def verify_deps(item, *dep_names):
        verify_equal(tuple(sorted(item.name for item in item.referenced)), dep_names)
    ...
    def verify_assignable_imp(item, assignable_no_modules, assignable_modules):
        ...
        for modules_val, assignable in (0, assignable_no_modules), (
            ...
            verify(
                ...
                "was {}.".format(item.name, module_msg, assignable, item.assignable),
            )
    ...
    verify(
        c.syms["A"].nodes[0].parent is c.top_node, "A's parent should be the top node"
    )
    ...
    verify(c.syms["E"].nodes[0].parent.item == MENU, "E's parent should be a menu")
    ...
    for sym in syms:
        verify(sym.user_value is None, "{} should not have a user value to begin with")
    ...
    for name in "A", "B", "C", "D":
        verify(
            c.syms[name].choice is not None, "{} should be a choice symbol".format(name)
        )
    ...
    for name in (
        ...
        verify(
            c.syms[name].choice is None, "{} should not be a choice symbol".format(name)
        )
...
""",
    ...
    for c in Kconfig("Kconfiglib/tests/Kmisc", warn=False), Kconfig(
        ...
        for item in (
            ...
            verify(item.kconfig is c, ".kconfig not properly set for " + repr(item))
    ...
    def select_and_verify(sym):
        ...
        verify(sym.choice.selection is sym, sym.name + " should be the selected symbol")
        ...
        verify(sym.tri_value == 2, sym.name + " should have value y when selected")

        verify(
            sym.user_value == 2, sym.name + " should have user value y when selected"
        )
    ...
    verify(
        choice.selection is c.syms["OPT_1"], "User selection should override defaults"
    )

    verify(
        c.named_choices["DEFAULTS_NOT_VISIBLE"].selection is c.syms["OPT_8"],
        "Non-visible choice symbols should cause the next default to be " "considered",
    )
    ...
    for name in "MMT_1", "MMT_2", "MMT_4", "MMT_5":
        verify(
            c.syms[name].orig_type == BOOL, "Expected {} to get type bool".format(name)
        )

    verify(
        c.syms["MMT_3"].orig_type == TRISTATE, "Expected MMT_3 to have type tristate"
    )
    ...
    for i in range(11):
        ...
        try:
            ...
            if "Dependency loop" not in str(e):
                fail("dependency loop in {} raised wrong KconfigError".format(filename))
        except:
            fail("dependency loop in {} raised wrong exception".format(filename))
        else:
...
"""[:-1],
    ...
    verify_variable("quote", '"$(1)" "$(2)"', '"one" "two"', True, "one", "two")
    verify_variable(
...
""",
    ...
    verify_repr(
        "simple-immediate", "<variable simple-immediate, immediate, value 'bar'>"
    )
    ...
    verify_variable("safe-fn-rec-res", "$(safe-fn-rec,safe-fn-rec-2)", "foo", True)
    ...
    verify_variable(
        "shell-res", "$(shell,false && echo foo bar || echo baz qaz)", "baz qaz", True
    )
    ...
    verify_variable(
        "one-or-more-three", "$(one-or-more,foo,bar,baz)", "foo + bar,baz", True
    )
...
def run_conf_and_compare(script, conf_flag, arch):
    # Runs a Kconfiglib script via 'make scriptconfig', then runs the C
    # implementation with 'conf_flag', and compares the resulting .config files.

    shell(
        "make scriptconfig SCRIPT={} " "PYTHONCMD='{}'".format(script, sys.executable)
    )
    shell("mv .config ._config")
    shell("scripts/kconfig/conf --{} Kconfig".format(conf_flag))
...
def test_allnoconfig(arch, srcarch):
    ...
    'make scriptconfig'.
    """
    run_conf_and_compare("Kconfiglib/allnoconfig.py", "allnoconfig", arch)
...
def test_allnoconfig_walk(arch, srcarch):
    ...
    'make scriptconfig'.
    """
    run_conf_and_compare("Kconfiglib/examples/allnoconfig_walk.py", "allnoconfig", arch)
...
def test_allmodconfig(arch, srcarch):
    ...
    'make scriptconfig'.
    """
    run_conf_and_compare("Kconfiglib/allmodconfig.py", "allmodconfig", arch)
...
def test_allyesconfig(arch, srcarch):
    ...
    'make scriptconfig'.
    """
    run_conf_and_compare("Kconfiglib/allyesconfig.py", "allyesconfig", arch)
...
def test_sanity(arch, srcarch):
    ...
    for key, sym in kconf.syms.items():
        ...
        verify(
            sym not in kconf.const_syms, sym.name + " in both 'syms' and 'const_syms'"
        )

        for dep in sym._dependents:
            verify(
                not dep.is_constant,
                "the constant symbol {} depends on {}".format(dep.name, sym.name),
            )
    ...
    for key, sym in kconf.const_syms.items():
        verify(isinstance(key, str), "weird key '{}' in const_syms dict".format(key))
        ...
        verify(not sym.nodes, '"{}" is constant but has menu nodes'.format(sym.name))

        verify(
            not sym._dependents,
            '"{}" is constant but is a dependency of some symbol'.format(sym.name),
        )

        verify(not sym.choice, '"{}" is constant and a choice symbol'.format(sym.name))
    ...
    for choice in kconf.choices:
        for sym in choice.syms:
            ...
            verify(
                ...
                "{} is a choice symbol but is not a bool/tristate".format(sym.name),
            )
    ...
    while 1:
        ...
        node.__str__()
        verify(
            isinstance(node.item, (Symbol, Choice)) or node.item in (MENU, COMMENT),
...
def test_alldefconfig(arch, srcarch):
    ...
    'make scriptconfig'.
    """
    run_conf_and_compare("Kconfiglib/alldefconfig.py", "alldefconfig", arch)
...
def test_defconfig(arch, srcarch):
    ...
    for defconfig in defconfigs:
        ...
        shell("scripts/kconfig/conf --defconfig='{}' Kconfig".format(defconfig))
